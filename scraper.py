...
def get_group_members_count(group_id: int) -> int:
    ...
    response = urllib.request.urlopen(url=group_info_api_url)
...
def get_group_members(group_id: int, timeout: urlopen_timeout = DEFAULT_TIMEOUT_GET_GROUP_MEMBERS) -> Set[int]:
    ...
    while True:
        ...
        response = urllib.request.urlopen(url=url_with_params, timeout=timeout)
...
def get_user_groups_ids(user_id: int, timeout: urlopen_timeout = DEFAULT_TIMEOUT_GET_USER_GROUPS_IDS) -> FrozenSet[int]:
    LOG(f"Getting groups IDs - user '{user_id}'", 4)
    ...
    response: urllib.request._UrlopenRet = urllib.request.urlopen(url=user_groups_api_url, timeout=timeout)
    status_code: int = response.getcode()
...
def get_group_name_from_id(group_id: int, timeout: urlopen_timeout = DEFAULT_TIMEOUT_GET_GROUP_NAME):
    ...
    response = urllib.request.urlopen(url=api_url, timeout=timeout)
    status_code: int = response.getcode()
